package com.kozen.support.x.utils

import com.kozen.financial.aidl.emv.EmvCapk

/**
 * @description:
 * @projectName:
 * @author:Yao.Zhang
 * @date :2025/2/11 10:15
 * @version 1.0.0
 */
object CapkUtil {

    fun createCapkList(): MutableList<EmvCapk> {
        val capkList = mutableListOf<EmvCapk>()

        capkList.add(
            buildCapkParam(
                "A000000003",
                "01",
                "",
                "03",
                "D34A6A776011C7E7CE3AEC5F03AD2F8CFC5503CC"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "07",
                "",
                "03",
                "B4BC56CC4E88324932CBC643D6898F6FE593B172"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "08",
                "",
                "03",
                "20D213126955DE205ADC2FD2822BD22DE21CF9A8"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "09",
                "",
                "03",
                "1FF80A40173F52D7D27E0F26A146A1C8CCB29046"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "57",
                "942B7F2BA5EA307312B63DF77C5243618ACC2002BD7ECB74D821FE7BDC78BF28F49F74190AD9B23B9713B140FFEC1FB429D93F56BDC7ADE4AC075D75532C1E590B21874C7952F29B8C0F0C1CE3AEEDC8DA25343123E71DCF86C6998E15F756E3",
                "010001",
                "251A5F5DE61CF28B5C6E2B5807C0644A01D46FF5"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "92",
                "",
                "03",
                "429C954A3859CEF91295F663C963E582ED6EB253"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "95",
                "",
                "03",
                "EE1511CEC71020A9B90443B37B1D5F6E703030F6"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "94",
                "",
                "03",
                "C4A3C43CCF87327D136B804160E47D43B60E6E0F"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000003",
                "99",
                "",
                "03",
                "4ABFFD6B1C51212D05552E431C5B17007D2F5E6D"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "0C",
                "",
                "03",
                "D7DD7AAC8B67A9A3CC72F35F5D96F265E16EB3FE"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "0B",
                "",
                "03",
                "BD331F9996A490B33C13441066A09AD3FEB5F66C"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "0A",
                "",
                "03",
                "C88BE6B2417C4F941C9371EA35A377158767E4E3"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "85",
                "",
                "03",
                "96C22F92B7644934F03B4065F1C37BC9DBEA45B0"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "84",
                "",
                "03",
                "4D4E6D415F2CF8C394D40C49FB2459110578CF22"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "83",
                "",
                "03",
                "1CC9BA05BC70F3D049F817404051122E35AC9683"
            )
        )

        capkList.add(
            buildCapkParam(
                "A000000333",
                "80",
                "",
                "010001",
                "A35663495B5702D2ADE19104ADB8EE07F97E8B2A"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "09",
                "",
                "03",
                "A075306EAB0045BAF72CDD33B3B678779DE1F527"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "08",
                "",
                "03",
                "EE23B616C95C02652AD18860E48787C079E8E85A"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "04",
                "",
                "03",
                "F527081CF371DD7E1FD4FA414A665036E0F5E6E5"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "03",
                "",
                "03",
                "87F0CD7C0E86F38F89A66F8C47071A8B88586F26"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000333",
                "02",
                "",
                "03",
                "03BB335A8549A03B87AB089D006F60852E4B8060"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "00",
                "",
                "03",
                "EC0A59D35D19F031E9E8CBEC56DB80E22B1DE130"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "02",
                "",
                "03",
                "33408B96C814742AD73536C72F0926E4471E8E47"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "03",
                "",
                "03",
                "5ADDF21D09278661141179CBEFF272EA384B13BB"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "04",
                "",
                "03",
                "381A035DA58B482EE2AF75F4C3F2CA469BA4AA6C"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "05",
                "",
                "03",
                "EBFA0D5D06D8CE702DA3EAE890701D45E274C845"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "06",
                "",
                "03",
                "F910A1504D5FFB793D94F3B500765E1ABCAD72D9"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "EF",
                "",
                "03",
                "21766EBB0EE122AFB65D7845B73DB46BAB65427A"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F1",
                "",
                "03",
                "D8E68DA167AB5A85D8C3D55ECB9B0517A1A5B4BB"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F3",
                "",
                "03",
                "A69AC7603DAF566E972DEDC2CB433E07E8B01A9A"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F4",
                "",
                "03",
                "BF6B5B9C47134E494571732A4903C935874682B9"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F5",
                "",
                "010001",
                "C2239804C8098170BE52D6D5D4159E81CE8466BF"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F6",
                "",
                "03",
                "502909ED545E3C8DBD00EA582D0617FEE9F6F684"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F7",
                "",
                "010001",
                "EEB0DD9B2477BEE3209A914CDBA94C1C4A9BDED9"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F8",
                "",
                "03",
                "F06ECC6D2AAEBF259B7E755A38D9A9B24E2FF3DD"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "F9",
                "",
                "03",
                "336712DCC28554809C6AA9B02358DE6F755164DB"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "FA",
                "",
                "03",
                "5BED4068D96EA16D2D77E03D6036FC7A160EA99C"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000004",
                "FE",
                "",
                "03",
                "9A295B05FB390EF7923F57618A9FDA2941FC34E0"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "5A",
                "",
                "03",
                "CC9585E8E637191C10FCECB32B5AE1B9D410B52D"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "5B",
                "",
                "03",
                "4DC5C6CAB6AE96974D9DC8B2435E21F526BC7A60"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "5C",
                "",
                "03",
                "60154098CBBA350F5F486CA31083D1FC474E31F8"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "5D",
                "",
                "03",
                "B51EC5F7DE9BB6D8BCE8FB5F69BA57A04221F39B"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "03",
                "",
                "03",
                "8708A3E3BBC1BB0BE73EBD8D19D4E5D20166BF6C"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "04",
                "D0F543F03F2517133EF2BA4A1104486758630DCFE3A883C77B4E4844E39A9BD6360D23E6644E1E071F196DDF2E4A68B4A3D93D14268D7240F6A14F0D714C17827D279D192E88931AF7300727AE9DA80A3F0E366AEBA61778171737989E1EE309",
                "03",
                "FDD7139EC7E0C33167FD61AD3CADBD68D66E91C5"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "0E",
                "",
                "03",
                "A7266ABAE64B42A3668851191D49856E17F8FBCD"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "0F",
                "",
                "03",
                "A73472B3AB557493A9BC2179CC8014053B12BAB4"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "10",
                "",
                "03",
                "C729CF2FD262394ABC4CC173506502446AA9B9FD"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "65",
                "",
                "03",
                "894C5D08D4EA28BB79DC46CEAD998B877322F416"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "97",
                "",
                "03",
                "EBDA522B631B3EB4F4CBFC0679C450139D2B69CD"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "98",
                "",
                "03",
                "D4DBA428CF11D45BAEB0A35CAEA8007AD8BA8D71"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "99",
                "",
                "03",
                "94790D020F4F692D59289F36451872078005B63B"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C1",
                "",
                "03",
                "DC79D6B5FC879362299BC5A637DAD2E0D99656B8"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C2",
                "",
                "03",
                "8E748296359A7428F536ADDA8E2C037E2B697EF6"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C3",
                "",
                "03",
                "12F1790CB0273DC73C6E70784BC24C12E8DB71F6"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C7",
                "",
                "03",
                "6221E0C726BAC8F8AC25F8F93B811D1FFD4C131C"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C8",
                "",
                "03",
                "33BD7A059FAB094939B90A8F35845C9DC779BD50"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "C9",
                "",
                "03",
                "8E8DFF443D78CD91DE88821D70C98F0638E51E49"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000025",
                "CA",
                "",
                "03",
                "6BDA32B1AA171444C7E8F88075A74FBFE845765F"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "08",
                "",
                "03",
                "DD36D5896228C8C4900742F107E2F91FE50BC7EE"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "09",
                "",
                "03",
                "4410C6D51C2F83ADFD92528FA6E38A32DF048D0A"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "0F",
                "",
                "03",
                "2A1B82DE00F5F0C401760ADF528228D3EDE0F403"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "10",
                "",
                "03",
                "C75E5210CBE6E8F0594A0F1911B07418CADB5BAB"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "11",
                "",
                "03",
                "D9FD62C9DD4E6DE7741E9A17FB1FF2C5DB948BCB"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "12",
                "",
                "03",
                "874B379B7F607DC1CAF87A19E400B6A9E25163E8"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "13",
                "",
                "03",
                "54CFAE617150DFA09D3F901C9123524523EBEDF3"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000065",
                "14",
                "",
                "03",
                "C0D15F6CD957E491DB56DCDD1CA87A03EBE06B7B"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "03",
                "",
                "03",
                "CA1E9099327F0B786D8583EC2F27E57189503A57"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "04",
                "",
                "03",
                "17F971CAF6B708E5B9165331FBA91593D0C0BF66"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000152",
                "05",
                "",
                "03",
                "12BCD407B6E627A750FDF629EE8C2C9CC7BA636A"
            )
        )
        //  A000000371
        capkList.add(
            buildCapkParam(
                "A000000371",
                "06",
                "",
                "03",
                "F5BAB84ECE5F8BD45511E5CA861B80C7E6C51F55"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000371",
                "05",
                "",
                "03",
                "676822D335AB0D2C3848418CB546DF7B6A6C32C0"
            )
        )
        capkList.add(
            buildCapkParam(
                "A000000371",
                "04",
                "",
                "03",
                "8B36A3E3D814CE6C6EBEAAF27674BB7BC67275B1"
            )
        )
        return capkList
    }

    private fun buildCapkParam(
        rid: String,
        capkIndex: String,
        modules: String,
        exponents: String,
        checksum: String,
    ): EmvCapk {
        return EmvCapk().apply {
            RID = ByteUtils.hexStringToBytes(rid)
            CapkIndex = ByteUtils.hexStringToBytes(capkIndex).get(0)
            Module = ByteUtils.hexStringToBytes(modules)
            Exponent = ByteUtils.hexStringToBytes(exponents)
            Checksum = ByteUtils.hexStringToBytes(checksum)
            AlgorithmInd = EmvCapk.ALGO_IND_RSA.toByte()
            HashInd = EmvCapk.HASH_IND_SHA1.toByte()
        }
    }
}